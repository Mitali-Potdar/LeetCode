    ...
    int numBusesToDestination(vector<vector<int>>& routes, int source, int target) {
        ...
        while(!track.empty()){
            ...
            for(auto busesavail : adj[station]){ // buses available at a station
                if(bustaken.count(busesavail) == 0){ // Exclude the buses which have not been taken
                    for(auto reach : routes[busesavail]){ // stations which can be reached from that station
                        if(stationsvisited.count(reach) == 0){ // Stations which you have already visited exclude them
